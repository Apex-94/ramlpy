...
class Raml08Parser(BaseParser):
    """Parser for RAML 0.8 files."""
    
    def parse(self, text):
        """Parse RAML 0.8 text into AST."""
        data = load_yaml(text)
    ...
    def _parse_document(self, data):
        ...
        return DocumentNode(version='0.8', data={
    ...
    def _parse_resource(self, path, data):
        """Parse a resource and its nested resources."""
        if data is None:
            data = {}
        methods = []
        nested = []
    ...
    def _parse_parameters(self, params):
        ...
        for name, spec in params.items():
            # Handle RAML shorthand: paramName?: type
